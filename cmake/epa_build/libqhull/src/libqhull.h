...
/*===== symbol visibility support for Windows or gcc compilers =====*/
#ifdef USINGDLL
  #if defined ( WIN32 )
/* Visual C/C++, Borland, MinGW and Watcom */
    ...
    #else
      #define QHULLDLLEXPORT
      #define QHULLDLLIMPORT
    #endif
 ...
 * QHULLDLLEXPORT to be visible.  */
    #define QHULLDLLEXPORT    __attribute__ ( ( visibility( "default" ) ) )
    #define QHULLDLLIMPORT
  #endif
#endif

/* For an unknown compiler or static build we clear the macros */
#ifndef QHULLDLLEXPORT
  #define QHULLDLLEXPORT
  #define QHULLDLLIMPORT
#endif

#if defined ( qhull_EXPORTS )
  #define QHULLDLLIMPEXP    QHULLDLLEXPORT
  #define QHULLDLLIMPEXP_DATA( type )    QHULLDLLEXPORT type
#else
  #define QHULLDLLIMPEXP    QHULLDLLIMPORT
  #define QHULLDLLIMPEXP_DATA( type )    QHULLDLLIMPORT type
#endif

/*============ constants and basic types ====================*/

extern const QHULLDLLIMPEXP_DATA( char * ) qh_version; /* defined in global.c */
...
#if qh_QHpointer
#define qh qh_qh->
extern QHULLDLLIMPEXP_DATA( qhT *) qh_qh;     /* allocated in global.c */
#else
#define qh qh_qh.
extern QHULLDLLIMPEXP_DATA( qhT ) qh_qh;
#endif
...
QHULLDLLIMPEXP void    qh_qhull(void);
QHULLDLLIMPEXP boolT   qh_addpoint(pointT *furthest, facetT *facet, boolT checkdist);
QHULLDLLIMPEXP void    qh_printsummary(FILE *fp);

/********* -user.c prototypes (alphabetical) **********************/

QHULLDLLIMPEXP void    qh_errexit(int exitcode, facetT *facet, ridgeT *ridge);
QHULLDLLIMPEXP void    qh_errprint(const char* string, facetT *atfacet, facetT *otherfacet, ridgeT *atridge, vertexT *atvertex);
QHULLDLLIMPEXP int     qh_new_qhull(int dim, int numpoints, coordT *points, boolT ismalloc,
                char *qhull_cmd, FILE *outfile, FILE *errfile);
QHULLDLLIMPEXP void    qh_printfacetlist(facetT *facetlist, setT *facets, boolT printall);
QHULLDLLIMPEXP void    qh_printhelp_degenerate(FILE *fp);
QHULLDLLIMPEXP void    qh_printhelp_narrowhull(FILE *fp, realT minangle);
QHULLDLLIMPEXP void    qh_printhelp_singular(FILE *fp);
QHULLDLLIMPEXP void    qh_user_memsizes(void);

/********* -usermem.c prototypes (alphabetical) **********************/
QHULLDLLIMPEXP void    qh_exit(int exitcode);
QHULLDLLIMPEXP void    qh_free(void *mem);
QHULLDLLIMPEXP void   *qh_malloc(size_t size);

/********* -userprintf.c prototypes (alphabetical) **********************/
QHULLDLLIMPEXP void    qh_fprintf(FILE *fp, int msgcode, const char *fmt, ... );
QHULLDLLIMPEXP void    qh_fprintf_rbox(FILE *fp, int msgcode, const char *fmt, ... );

/***** -geom.c/geom2.c/random.c prototypes (duplicated from geom.h, random.h) ****************/

QHULLDLLIMPEXP int     qh_argv_to_command(int argc, char *argv[], char* command, int max_size);
QHULLDLLIMPEXP int     qh_argv_to_command_size(int argc, char *argv[]);
QHULLDLLIMPEXP facetT *qh_findbest(pointT *point, facetT *startfacet,
                     boolT bestoutside, boolT newfacets, boolT noupper,
                     realT *dist, boolT *isoutside, int *numpart);
QHULLDLLIMPEXP facetT *qh_findbestnew(pointT *point, facetT *startfacet,
                     realT *dist, boolT bestoutside, boolT *isoutside, int *numpart);
QHULLDLLIMPEXP boolT   qh_gram_schmidt(int dim, realT **rows);
QHULLDLLIMPEXP void    qh_outerinner(facetT *facet, realT *outerplane, realT *innerplane);
QHULLDLLIMPEXP void    qh_printsummary(FILE *fp);
QHULLDLLIMPEXP void    qh_projectinput(void);
QHULLDLLIMPEXP void    qh_randommatrix(realT *buffer, int dim, realT **row);
QHULLDLLIMPEXP void    qh_rotateinput(realT **rows);
QHULLDLLIMPEXP void    qh_scaleinput(void);
QHULLDLLIMPEXP void    qh_setdelaunay(int dim, int count, pointT *points);
QHULLDLLIMPEXP coordT  *qh_sethalfspace_all(int dim, int count, coordT *halfspaces, pointT *feasible);

/***** -global.c prototypes (alphabetical) ***********************/

QHULLDLLIMPEXP unsigned long qh_clock(void);
QHULLDLLIMPEXP void    qh_checkflags(char *command, char *hiddenflags);
QHULLDLLIMPEXP void    qh_clear_outputflags(void);
QHULLDLLIMPEXP void    qh_freebuffers(void);
QHULLDLLIMPEXP void    qh_freeqhull(boolT allmem);
QHULLDLLIMPEXP void    qh_freeqhull2(boolT allmem);
QHULLDLLIMPEXP void    qh_init_A(FILE *infile, FILE *outfile, FILE *errfile, int argc, char *argv[]);
QHULLDLLIMPEXP void    qh_init_B(coordT *points, int numpoints, int dim, boolT ismalloc);
QHULLDLLIMPEXP void    qh_init_qhull_command(int argc, char *argv[]);
QHULLDLLIMPEXP void    qh_initbuffers(coordT *points, int numpoints, int dim, boolT ismalloc);
QHULLDLLIMPEXP void    qh_initflags(char *command);
QHULLDLLIMPEXP void    qh_initqhull_buffers(void);
QHULLDLLIMPEXP void    qh_initqhull_globals(coordT *points, int numpoints, int dim, boolT ismalloc);
QHULLDLLIMPEXP void    qh_initqhull_mem(void);
QHULLDLLIMPEXP void    qh_initqhull_outputflags(void);
QHULLDLLIMPEXP void    qh_initqhull_start(FILE *infile, FILE *outfile, FILE *errfile);
QHULLDLLIMPEXP void    qh_initqhull_start2(FILE *infile, FILE *outfile, FILE *errfile);
QHULLDLLIMPEXP void    qh_initthresholds(char *command);
QHULLDLLIMPEXP void    qh_option(const char *option, int *i, realT *r);
#if qh_QHpointer
QHULLDLLIMPEXP void    qh_restore_qhull(qhT **oldqh);
QHULLDLLIMPEXP qhT    *qh_save_qhull(void);
#endif
...
QHULLDLLIMPEXP void qh_meminit(FILE *ferr);
QHULLDLLIMPEXP void qh_memfreeshort(int *curlong, int *totlong);

/********* -poly.c/poly2.c prototypes (duplicated from poly.h) **********************/

QHULLDLLIMPEXP void    qh_check_output(void);
QHULLDLLIMPEXP void    qh_check_points(void);
...
QHULLDLLIMPEXP int     qh_pointid(pointT *point);
QHULLDLLIMPEXP setT   *qh_pointvertex(void /*qh.facet_list*/);
QHULLDLLIMPEXP void    qh_setvoronoi_all(void);
QHULLDLLIMPEXP void    qh_triangulate(void /*qh facet_list*/);

/********* -rboxpoints.c prototypes **********************/
QHULLDLLIMPEXP int     qh_rboxpoints(FILE* fout, FILE* ferr, char* rbox_command);
QHULLDLLIMPEXP void    qh_errexit_rbox(int exitcode);

/********* -stat.c prototypes (duplicated from stat.h) **********************/

QHULLDLLIMPEXP void    qh_collectstatistics(void);
QHULLDLLIMPEXP void    qh_printallstatistics(FILE *fp, const char *string);
