...
program x31f95
  ...
  real(kind=plflt) a
  integer(kind=plint) r1(2_plint), g1(2_plint), b1(2_plint)
  data r1 /0, 255/
  data g1 /255, 0/
  data b1 /0, 0/
  real(kind=plflt) a1(2_plint)
...
!     Retrieve and print the name of the output file (if any)
  call plgfnam(fnam)
  if (len(trim(fnam)) .eq. 0_plint) then
...
!     value
  ...
  call plscolbg(10_plint,20_plint,30)
  call plgcolbg(r, g, b)
  write(*,'(A,3I3)') 'background colour parameters: r, g, b =', r, g, b
  if (r .ne. 10_plint .or. g .ne. 20_plint .or. b .ne. 30_plint) then
     write(stderr,*) 'plgcolbg test failed'
     status = 1_plint
  endif

  call plscolbga(20_plint,30_plint,40,0.5_plflt)
