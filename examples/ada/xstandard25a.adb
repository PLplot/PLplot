     ...
     for k in 0 .. 1 loop
       for j in 0 .. 3 loop
            ...
            for i in xextreme'range(1) loop
                ...
                Set_Viewport_World(xextreme(i, 0), xextreme(i, 1), yextreme(i, 0), yextreme(i, 1));
                ...
                begin
                    ...
                    if k = 0 then
                        Fill_Polygon(x0_Stop_Sucking, y0_Stop_Sucking);
                    else
                        Fill_Polygon_Gradient(x0_Stop_Sucking, y0_Stop_Sucking, 45.0);
                    end if;
                    Set_Pen_Color(Yellow);
                    Select_Line_Style(1);
                    Draw_Curve(x0_Stop_Sucking, y0_Stop_Sucking);
                end;
            end loop; -- i
        end loop; -- j
    end loop; -- k
