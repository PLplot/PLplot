...
proc help_gui {{w .help_gui}} {
    ...
    insertWithTags $w.t {\
The PLPLOT/TK user interface is constructed from TK widgets, using Tcl as 
a (dynamic) specification language.  The Tcl scripts provided with PLPLOT
...
embedded interpreters to send commands back and forth (note: an extended
version of Tcl called Tcl-DP allows sockets to be used for this, and the
model will eventually be extended to allow distribution across a network).
The existence of an interpreter in each process (actually each plplot stream
that uses the TK driver) allows for a very easy and robust method of sending
commands between processes.  The data goes through via an alternate channel
(fifo for now, maybe socket later) for maximum speed.

Also supported (but as of yet untested) are some alternate configurations:
...
(b) the server creates widgets, then calls functions to do the actual
      ...
      user interface goes "dead" while the calculation is proceeding (not a
...
functions calls in your own program.  (Note: these capabilities have not been
