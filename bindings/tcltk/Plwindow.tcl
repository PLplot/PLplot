...
itcl::class Plwindow {
    ...
    method use_cpptcl {{id ""}} {
	# requires the use of Cpplplot, Vince's Plplot wrapper.
	if {$id != ""} {
	    if {[info commands ::pl$id] == ""} {
		set cpp [cppmeta create pltcl pl$id]
		return pl$id
	    }
	}
	set i 0
	while {[info commands ::pl$id$i] != ""} {
	    incr i
	}
	set cpp [cppmeta create pltcl pl$id$i]
    }
