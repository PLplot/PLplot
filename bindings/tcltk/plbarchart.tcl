## -*-Tcl-*-
 ...
 #                                last update: 11/19/1999 {23:21:24 PM} 
 #  Author: Vince Darley
...
itcl::class Plbarchart {
    ...
    method cppcmd {args} {uplevel 1 $itk_component(pl) cppcmd $args }
...
itcl::body Plbarchart::datapoints {data} {
	...
	cppcmd col 0
	cppcmd hist [_pp$nn info] _pp$nn $minbin $maxbin $bins 0
    }
    cppmeta create matrix _pp$nn f [llength $data] = $data
    cppcmd col 1
    cppcmd hist [_pp$nn info] _pp$nn $minbin $maxbin $bins 0
}
...
itcl::body Plbarchart::initialise {} {
    ...
    drawAxes
    cppmeta create matrix _pv$nn f [llength $V] = $V
}
