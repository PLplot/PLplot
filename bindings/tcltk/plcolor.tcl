...
#
# 
# Revision 1.9 1998 Vince, Uses [incr Tcl] (or tcl++ emulation)
# 
...
itcl::class Buttons1 {
	...
	frame $w
	button $w.savep -text "Save Palette" \
	    -command [code $this savePalette]
	button $w.loadp -text "Load Palette" \
	    -command [code $this loadPalette]
	button $w.ok -text "OK" \
...
itcl::class ColorPalette1 {
	...
	if {$file != {}} {
	    ...
	    destroy .edit.palette
	    pack append .edit \
		[$this create .edit.palette $plot] "left filly"
	}
    }
    method inc {i inc} {
...
#	puts stdout "$plot cmd plscol1 $i $color $plcmap1_pos($i) $plcmap1_rev($i)"
...
#	puts stdout "$plot cmd plscmap1 $ncol1 $cmap1"
