...
#
# 
# Vince's Changes 1996-1998:
# 
# Rewrote for Tk4 or newer scrollbar commands.  This is required for
# my changes to plframe.c, and makes scrolling code a bit simpler,
# and more obvious for those not immersed in old versions of Tk.
# 
# Fixed nasty bug.  The moral: depending upon whether 'tearoff' is
# the default or not, menus are automatically given a '0' index 
# tearoff item.  Therefore even if you know the entries in your
# menus, unless you wish to force the 'tearoff' option one way or 
# another (not nice to your users), you have to index items by value,
# not number.  The bug here was caused by the 'orient' -postcommand
# invoking the zeroth entry, which tried to tear off the menu, which
# tried to call the postcommand again (since it was being torn off!)...
# Anyway I fixed the orient and zoom postcommands to work properly.
# 
# Modernised 'pack' commands to avoid use of obsolete 'append' etc.
# (Actually did this hoping it would resolve a bug on Windows, but
# no such luck)
# 
# Various fixes, put in a namespace for Tcl8.0 (now 'info commands pl*' 
# gives you just a few commands which is rather nice).  This only works
# correctly with Tcl8.0p2 or newer.
# 
# Changed some 'catch "command w. arguments like files which may have spaces"'
# to 'catch [list ...]' so that filenames with spaces work on MacOS/Win.
# 
# Widget uses 'toplevel -menu' option to use a real menu-bar.
# 
# Uses some more standard Tk dialogs for files etc.
# 
