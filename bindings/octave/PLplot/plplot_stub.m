...
function plclabel_format(lexp, sigdig)
% plclabel_format(lexp, sigdig)
%
  plplot_octave(0, lexp, sigdig);
endfunction

function plclabel_param(offset, size, spacing, active)
% plclabel_param(offset, size, spacing, active)
%
...
function plxormod(mode)
% plxormod(mode)
%
  plplot_octave(107, mode);
endfunction
...
function plshades(a, defined, left, right, bottom, top, clevel, fill_width, cont_color, cont_width, rectangular, tr)
% plshades(a, defined, left, right, bottom, top, clevel, fill_width, cont_color, cont_width, rectangular, tr)
%
  plplot_octave(123, a, defined, left, right, bottom, top, clevel, fill_width, cont_color, cont_width, rectangular, tr);
endfunction
...
%
  id = plplot_octave(124, xspec, yspec, xmin, xmax, xjump, ymin, ymax, xlpos, ylpos, y_ascl, acc, colbox, collab, colline, styline, legline1, legline2, legline3, legline4, labx, laby, labtop);
endfunction
...
% Process options list using current options info.  
  retval = plplot_octave(130, p_argc, argv, mode);
endfunction

function [p_h, p_l, p_s] = plRGB_HLS(r, g, b)
% [p_h, p_l, p_s] = plRGB_HLS(r, g, b)
%
  [p_h, p_l, p_s] = plplot_octave(131, r, g, b);
endfunction
...
% Process input strings, treating them as an option and argument pair.  
  retval = plplot_octave(133, opt, optarg);
endfunction
...
% Set the strings used in usage and syntax messages.  
  plplot_octave(134, program_string, usage_string);
endfunction

function plarrows(u, v, x, y, scale, dx, dy)
% plarrows(u, v, x, y, scale, dx, dy)
%
  plplot_octave(135, u, v, x, y, scale, dx, dy);
endfunction
...
% Converts input values from relative plot coordinates to relative  device coordinates.  
  plplot_octave(137, xmin, ymin, xmax, ymax);
endfunction
...
% Set the variables to be used for storing error info
  plplot_octave(138, errcode, errmsg);
endfunction
