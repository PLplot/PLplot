 ...
 * pltk_AreWeGrayScale now takes a pointer to the enclosing PlFrame, and
 ...
 * will be necessary. 
 *
 * Major changes: added 'plFramePtr' to the PLStream, since this driver
 * must now get its info from Tk (hence from the PlFrame) rather than
 * directly from the X system. Removed all top-level win support code --
 * this driver must be inside a PlFrame or it won't work.  This is simpler
...
This device driver is designed to be used by a PlFrame, and in fact requires
the existence of an enclosing PlFrame.
...
/* Dummy definition of PLFrame containing first few fields */
...
int pltk_AreWeGrayscale(PlFrame *plf);
 ...
 *
 * Initialize device.
 * Tk-dependent stuff done in plD_open_tkw() and Init().
\*--------------------------------------------------------------------------*/

void
plD_init_tkw(PLStream *pls)
{
    TkwDev *dev;
    float pxlx, pxly;
    int xmin = 0;
...
plD_open_tkw(PLStream *pls)
{
...
#if defined(MAC_TCL) || defined(__WIN32__)
	if(!pls->FileName) {
	    pls->FileName = TkGetDefaultScreenName(NULL,NULL);
	}
	tkwd->display = pls->plFramePtr->display;
#else
	...
	tkwd->map = Tk_Colormap(pls->plFramePtr->tkwin);
	GetVisual(pls);
	...
	if (pls->colorset)
	    ...
	    pls->color = 1;
	    tkwd->color = ! pltk_AreWeGrayscale(pls->plFramePtr);
	}
...
void
plD_line_tkw(PLStream *pls, short x1a, short y1a, short x2a, short y2a)
{
...
void
plD_polyline_tkw(PLStream *pls, short *xa, short *ya, PLINT npts)
{
    TkwDev *dev = (TkwDev *) pls->dev;
    TkwDisplay *tkwd = (TkwDisplay *) dev->tkwd;
...
WaitForPage(PLStream *pls)
{
    ...
    if (plf == NULL) {
		plwarn("WaitForPage: Illegal call --- driver can't find enclosing PlFrame");
		return;
    }
	PlframeAtEop(plf->interp, plf);

    while (!dev->exit_eventloop && !Tcl_InterpDeleted(plf->interp)) {
    	Tcl_DoOneEvent(0);
    }
    dev->exit_eventloop = FALSE;
}
...
void 
plD_state_tkw(PLStream *pls, PLINT op)
{
    TkwDev *dev = (TkwDev *) pls->dev;
    TkwDisplay *tkwd = (TkwDisplay *) dev->tkwd;
    XColor* xc;
...
void
plD_esc_tkw(PLStream *pls, PLINT op, void *ptr)
{
    TkwDev *dev = (TkwDev *) pls->dev;
    TkwDisplay *tkwd = (TkwDisplay *) dev->tkwd;
...
Init(PLStream *pls)
{
    ...
    plf = pls->plFramePtr;
    if (plf == NULL) {
	plwarn("Init: Illegal call --- driver can't find enclosing PlFrame");
	return;
    }
    ...
    plD_state_tkw(pls, PLSTATE_COLOR0);
...
CreatePixmap(PLStream *pls)
{
    TkwDev *dev = (TkwDev *) pls->dev;
    TkwDisplay *tkwd = (TkwDisplay *) dev->tkwd;
    Tk_Window tkwin = pls->plFramePtr->tkwin;
    int (*oldErrorHandler)();
...
GetVisual(PLStream *pls)
{
    ...
    tkwd->visual = Tk_GetVisual(pls->plFramePtr->interp, 
				pls->plFramePtr->tkwin, 
				"best",
				&depth, NULL);
    tkwd->depth = depth;
...
void Tkw_StoreColor(PLStream* pls, TkwDisplay* tkwd, XColor* col) {
    ...
    xc = Tk_GetColorByValue(pls->plFramePtr->tkwin,col);
    CopyColour(xc,col);
#endif 
}
 ...
 * Changed July 1996 by Vince: now uses Tk to check the enclosing PlFrame
\*--------------------------------------------------------------------------*/

int
pltk_AreWeGrayscale(PlFrame *plf)
{
