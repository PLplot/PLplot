...
void plD_init_cgm(PLStream *pls)
{
    ...
    if (pls->width<1) pls->width = 1;         /* set a legal line colour */
 ...
 */
   ...
   if (dev->fill_colour!=dev->last_fill_colour)
      {
...
cdSetLineType(dev->im_out,1);
cdSetLineWidth(dev->im_out,pls->width);
cdSetFillStyle(dev->im_out,1);
cdSetEdgeVis(dev->im_out,0);   /* This doesn't do anything, but it should */
cdSetEdgeWidth(dev->im_out,0);
