...
page on a plotter.  See \code{plpage} for more information.}
...
are associated with which number by the \code{plancol} routine if supported
...
is plotted out.  See Section \ref{contour} for more information.}
...
the transformation between array indices and world coordinates.  See
Section \ref{contour} for more information.}
...
transformation arrays \code{xg} and \code{yg}.  \xref{contour} for more
information.}
...
of the array from \code{kx} to \code{lx} and from \code{ky} to \code{ly}
is plotted out.  A transformation routine \code{pltr} is used to map
indicies within the array to the world coordinates.  See Section
...
through which the contour runs in terms of the array indicies.} \argu{ 
\code{tx, ty} (PLFLT *, output)}{Pointers to the world coordinates
...
\item
\code{1}: In addition to box and labels, draw the two axes
...
routine should only be called {\em after\/} the call to \code{plbox} 
(or \code{plbox3}) is complete.  See section \ref{annotate} for more
information.}
...
displayed.  See Section \ref{threed} on page \pageref{threed} for further
details.} 
...
or use a different method for shifting focus.  See also \code{plgra}.}
...
\code{plbox3} or \code{plot3d}.  See Section \ref{threed} on page
\pageref{threed} for a more complete description of three-dimensional
plotting.}
