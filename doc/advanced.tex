...
* Standard Window::	Setting up a Standard Window with \code{plenv}
...
12.0\mbox{\, V}$, we would call the function \code{plwind} as
follows:\refill
...
For example, let's suppose that we have set \code{digmax} $ = 5$ via
\code{plsyax}, and for our plot a label is generated at \code{y} $ =
0.0000478$.  In this case the actual field width is longer than
\code{digmax}, so PLplot switches to floating point.  In this
representation, the label is printed as simply $4.78$ with the $10^{-5}$
exponent placed separately.\refill
...
it will always be true that \code{digfix} $\le$ \code{digmax}.  The
PLplot defaults are set up such that good results are usually obtained
without user intervention.\refill
...
\section{Setting up a Standard Window with \code{plenv}}
...
$0.0\le\code{z}\le 8.0$, we could use the following statements:\refill
...
assuming the user had already set up arrays \code{r} and \code{theta} to
...
such that \code{xg(i, NY+1) = xg(i, 1)} and \code{yg(i, NY+1) = yg(i, 1)} so
