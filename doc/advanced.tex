...
recommended for this purpose, \rou{plstar}, which prompts at the console
for the output device type, and \rou{plstart} which expects to be
supplied the keyword of the device as an argument.  The old routine
\code{plbeg} can also be used for initialization, but use of this
...
(\code{plbeg} has only been retained for compatibility with old codes,
and will not be discussed further).\refill

Besides selecting the device, \code{plstar} and \code{plstart} allow the
...
number of subpages in the vertical direction.\refill
...
\rou{plsstrm} is used to switch between streams (note one may only write
...
\rou{plsasp}, which sets the global aspect ratio and must be called
prior to \code{plstar}.  An aspect ratio of 0.0 corresponds to
...
\c 12.0\mbox{\, V}$, we would call the function \code{plwind} as
\c follows:\refill
...
(\pxref{Escape Sequences}) which allow subscripts and superscripts to be
used in text.  They are described more fully later in this chapter.\refill
...
sequences start with a number symbol (\verb+#+).\refill
...
\begin{itemize}
   ...
   \item \verb+#gx+: Greek letter corresponding to Roman letter \code{x}
                     (see below)
...
\c As usual, we would like to refer to a three dimensional point \code{(X,
\c Y, Z)} in terms of some meaningful user-specified coordinate system.
...
\c $0.0\le\code{z}\le 8.0$, we could use the following statements:\refill
...
\c The values of the function are stored in a two-dimensional array
\c \code{z[][]} where the array element \code{z[i][j]} contains the value
\c of the function at the point $x_i$, $y_j$.  (The two-dimensional array
\c \code{z} is a vectored array instead of a fixed size array.  \code{z}
...
\c where \code{z} is the two-dimensional array of size
...
user-defined function to \code{plcont} (the case for
\code{plconf} is slightly different and discussed below).  This
function pointer is the last argument \code{pltr}.  This function must
be declared as type \code{void} in the module which calls \code{plcont}.
This transformation function must have the parameter list:\refill
...
\c transformation is required, you can simply set \code{tr[0]$ = $tr[4]$ =
\c $1.}, with other entries set to $0.$.  Alternately you can pass the
\c built-in function \code{pltr0} to \code{plcont}; this function merely
\c sets \code{*tx} to \code{x} and \code{*ty} to {y}, resulting in no net
\c transformation.\refill

\c As an example in setting up \code{tr[]}, suppose that the array \code{z}
