...
Many of the underlying concepts used in the PLplot subroutine package are
based on ideas used in Tim Pearson's PGPLOT package originally written in
VAX-specific Fortran-77.  Sze Tan of the University of Auckland originally
developed PLplot on an IBM PC, and subsequently transferred it to a number
of other machines.  
...
may be obtained by \code{ftp} from \code{hagar.ph.utexas.edu}
(128.83.179.27).  Login as user \code{anonymous}, set file transfer type
to binary, and get the newest PLplot archive in the \code{pub/}
subdirectory.  We will provide PLplot in both \code{zoo} and \code{tar}
